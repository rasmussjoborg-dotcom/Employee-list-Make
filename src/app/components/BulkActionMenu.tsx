...
import { ChevronLeft, ChevronRight, Search, ArrowRight, Calendar } from "lucide-react";
...
export function BulkActionMenu({ selectedAvatars }: { selectedAvatars: string[] }) {
  ...
  const [whenSelection, setWhenSelection] = useState<string | null>(null);
          ...
          <div className="flex items-center flex-1 overflow-hidden h-full justify-between">
                ...
                >
                  ...
                  {selectedDepartment && (
                    <motion.div
                      initial={{ opacity: 0, x: 20 }}
                      animate={{ opacity: 1, x: 0 }}
                      className="flex items-center gap-3 pl-6 h-full shrink-0"
                    >
                      <p className="font-[number:var(--font-weight-semi-bold)] text-foreground text-[length:var(--text-paragraph-sm)]">When:</p>
                      <ActionButton
                        label="Immediately"
                        isActive={whenSelection === "Immediately"}
                        onClick={() => setWhenSelection("Immediately")}
                      />
                      <button
                        onClick={() => setWhenSelection("Scheduled Date")}
                        className={`relative rounded-lg shrink-0 border border-border transition-colors cursor-pointer group ${whenSelection === "Scheduled Date" ? 'bg-muted/50' : 'hover:bg-muted/50'}`}
                      >
                        <div className="flex gap-[8px] items-center justify-center overflow-clip px-3 py-2 relative rounded-[inherit]">
                          <p className="font-[number:var(--font-weight-semi-bold)] leading-[20px] text-secondary-foreground text-[length:var(--text-paragraph-sm)]">Scheduled Date</p>
            ...
            {whenSelection && (
              <motion.div
                ...
                className="ml-auto pl-6 border-l border-border/50 h-full flex items-center pr-1"
              >
                <ApplyButton />
              </motion.div>
            )}
